...
#   git worktree root distinct from the primary project checkout. For the
#   treehouse get poll this is checked per sample against the project checkout AND
#   the firstmate home/root (treehouse setup hops through intermediate cwds;
#   mid-hop samples are rejected and polling continues), refusing if none appears
#   in 60s.
...
if [ "$KIND" != secondmate ] && [ "$BACKEND" != orca ]; then
  ...
  # worktree. Target the stable window id, not the name: if the name is ever lost
  # (e.g. an automatic-rename slips through), display-message -t <bad-name> falls
  # back to the active client's window, which would misread firstmate's OWN pane
  # path as the worktree and tangle a hook into the primary checkout. The window
  # id never lies.
  # The move is not atomic - treehouse's setup hops through intermediate cwds (the
  # user's home, the firstmate home) before settling - so "left the project" is not
  # enough: a sample landing mid-hop would record a wrong worktree= in meta and
  # install the crew's turn-end hook into the WRONG checkout. Accept a candidate
  # only once it proves it is a genuine ISOLATED worktree root: physically AT a git
  # worktree top, distinct from the project's primary checkout AND from the
  # firstmate home/root (a mis-sample can itself be another valid repo root; see
  # fm_isolated_worktree_root in fm-tangle-lib.sh). Branching/committing in a
  # mis-detected checkout would tangle it onto a feature branch, so anything else
  # keeps polling. This per-sample check is strictly stronger than
  # validate_spawn_worktree, so the treehouse path needs no second pass.
  wt_wait_secs=${FM_SPAWN_WT_TIMEOUT:-60}  # override is for tests; production keeps 60s
  for _ in $(seq 1 "$wt_wait_secs"); do
    p=$(spawn_current_path "$WT_TARGET" || true)
    if [ -n "$p" ] && fm_isolated_worktree_root "$p" "$PROJ_ABS_REAL" "$home_real" "$root_real" >/dev/null; then
      WT="$p"
      break
    fi
    sleep 1

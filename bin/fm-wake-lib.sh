...
fm_pid_identity() {
  ...
  # Pass -ww so the command column is never cut to the ambient terminal width: the
  # identity is written from a wide shell but re-read inside a narrow-COLUMNS hook,
  # where a truncated command would likewise reject a live watcher (issue #799).

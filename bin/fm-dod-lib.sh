...
Before you report done, read the PR back from the forge and confirm it is not a draft (\`gh-axi pr view <url>\` must print \`draft: no\`); if it is a draft, mark it ready with \`gh-axi pr ready\`.
...
After /no-mistakes reports CI green (the CI-ready return point - do not wait for it to keep monitoring in the background until merge), read the PR back from the forge and confirm it is not a draft (\`gh-axi pr view <url>\` must print \`draft: no\`); if it is a draft, mark it ready with \`gh-axi pr ready\`.
A draft cannot be merged, so a done report on one leaves the merge unasked.

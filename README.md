...
- `skills/` - public, installer-facing skills meant to be installed standalone into any project, independent of firstmate.
  ...
  Today that is `skills/stow` and `skills/paper-templates`, and the root `skills-lock.json` indexes them for the [agentskills.io](https://skills.sh) `skills` CLI.
  `skills/stow` is a generic session-knowledge-sweep skill that routes findings by explicit instruction first, then existing local conventions, then a private `.stow-notes.md` fallback, and curates tiered entries through decay, local archival, and user-approved on-demand offload proposals; it intentionally shares no code with the firstmate-internal `.agents/skills/stow` it is named after, so the two can evolve independently.
  `skills/paper-templates` produces paper-style PDF documents from an HTML source using two durable print templates (a two-column journal by default, a classic report as the alternate), a per-document transform, and a verified Paged.js render pipeline.

# End-user evidence: no-mistakes worker brief, Definition of done

Rendered by `fm-brief.sh <id> firstmate --mode no-mistakes` (the exact surface a dispatched worker reads).
Base bug: the DoD opened with "append `done: {summary}` ... and stop. Firstmate will then instruct you to run /no-mistakes", so workers stopped before validation.

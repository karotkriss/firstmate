...
`samples/` holds exactly that: a generic invented excerpt rendered in both templates (`samples/sample-a-p1.png`, `samples/sample-a-p2.png`, `samples/sample-c-p1.png`, `samples/sample-c-p2.png`), rebuilt by `samples/build_samples.py`, which also demonstrates the template-C productionizing recipe below.

Template A is production-ready as shipped.
Template C carries its own `@page` rules (A4, running header with the `%%SHORT%%` placeholder, folio, and a first-page suppression) but scopes its body rules under `.sC` so it can double as a sample-scale stylesheet; to productionize it, drop the `.sC` scoping prefix and keep `.cols { column-count: 2 }` for the body flow.

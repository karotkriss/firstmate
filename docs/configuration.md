...
So a hand-started row cannot exist without dispatch artifacts, [`bin/fm-tasks-axi.sh`](../bin/fm-tasks-axi.sh) refuses `add --start` and its `create --start` alias, because such a row would have no task record, status file, or inbox and would count as live work nobody is doing.

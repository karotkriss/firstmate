...
Dispatch is also the only path that places a row In flight: [`bin/fm-tasks-axi.sh`](../bin/fm-tasks-axi.sh) refuses `add --start`, because a hand-started row would have no task record, status file, or inbox and would count as live work nobody is doing.

...
For the `treehouse get` path that check runs per sampled pane path, and also excludes the firstmate home/root: treehouse setup hops through intermediate directories before settling, so mid-hop samples are rejected and polling continues until a genuine isolated worktree appears or the wait bound expires.

...
For ship and scout work, `fm-spawn.sh` waits for `treehouse get`, accepting a sampled pane path only once it is a real git worktree root distinct from both the project primary checkout and the firstmate home/root (treehouse setup hops through intermediate directories before settling, so mid-hop samples are rejected and polling continues), and refuses to launch if no such worktree appears within the wait bound.

...
- ~~Worktree-discovery isolation guard is symlink-fragile for a project path under a symlinked prefix (e.g. macOS's `/tmp` -> `/private/tmp`).~~ **Fixed** by the worktree-discovery-poll rework (`fm_isolated_worktree_root` in `bin/fm-tangle-lib.sh`).
  Discovered while building the runtime-backend-auto-detection real smoke test (`tests/fm-backend-autodetect-smoke.test.sh`), which needed a scratch project.
  ...
  When the project lived under a symlinked directory, the very first worktree-discovery poll saw two different strings for the identical starting directory, which - given the poll's old accept condition of merely "differs from the project" - made it accept the pane's very first sample as the worktree while it was still sitting in the project, then trip the isolation guard below it and false-refuse the spawn as "not isolated" before `treehouse get` ever moved the pane - backend-agnostic, not specific to herdr.
  `fm-spawn.sh` now resolves `PROJ_ABS`, `FM_HOME`, and `FM_ROOT` to their physical (`pwd -P`) forms before the poll, and `fm_isolated_worktree_root` resolves each sampled candidate the same way before comparing, so a symlinked project prefix no longer produces mismatched strings for the identical directory.

...
Once the exact pane is confirmed gone, teardown retires the task's own journal when it is a version 1 attempt or binds that same pane, because nothing remains for the session-start sweep to correlate; a journal bound to any other pane stays for that sweep.

...
`bin/fm-watch.sh`'s `state/.last-watcher-beat` marks phase progress, not cycle completion: the watcher touches it at the top of every cycle and again at each phase boundary once it is a tenth of the grace old, so a long cycle that is still advancing never reads stale, while a healthy watcher's beacon can still legitimately age up to `FM_POLL` seconds across its terminal wait (`event_wait_or_sleep`).

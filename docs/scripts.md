...
| `fm-brief.sh`            | Scaffold a ship brief with a worktree-isolation assertion, a report-only scout brief with `--scout`, a secondmate charter with `--secondmate`, or a two-phase propose/implement ship brief with `--phase propose\|implement` |

...
Each portable serial shard also passes `bin/fm-test-run.sh`'s `--per-script-timeout-secs 900`, well above the longest measured script and safely under the shard's 30-minute job cap, so a hung script is reported as one named failure instead of consuming the whole shard.

...
It never tears down a task, merges a pull request or merge request, dispatches new work, or mutates any task state as a side effect of producing the brief - those belong to the captain's explicit word and the normal task lifecycle.
...
1. **Gather live fleet state with one deterministic command.**
   ...
   Relay an unverified row as unverified rather than as open, and put a recorded forge item in Captain's Call only when the opt-in verifies its state and current evidence separately shows that it is ready to merge.
...
2. **Compose the detailed report file around the four-section spine, adding the richer detail the chat leaves out.**
   ...
   - **Captain's Call** - every open decision summarized with its options from the structured decision record, plus each verified pull request or merge request ready to merge and each needed credential or login, every forge item with the full `https://...` URL, never a bare `#number`.
...
1. **Captain's Call** - ONLY items that need the captain's own action now: a decision to make, a verified pull request or merge request to approve or merge, a credential or login to provide, or a blocker only the captain can clear.
   Empty-state: "Nothing needs your action right now."
2. **Recently Landed** - the bounded current recent-completions baseline: merged pull requests and merge requests, completed scouts, and finished local-only merges across the main fleet and every registered secondmate home.
...
- The chat follows `AGENTS.md` section 9 and carries one scannable line per item, each pull request or merge request as the full `https://...` URL; detailed decisions, plans, full gate reasons, and evidence live only in the report file, which the chat links to, so the chat stays materially shorter than that file.
...
- Every pull request or merge request reference is a full `https://...` URL, never a bare `#number`; a shorthand `#number` is fine only as a back-reference after the full URL has already appeared in the same report.
...
Do not tear down a task, merge a pull request or merge request, dispatch queued work, or mutate any `state/` or `data/` file other than the single report file as a side effect of generating the brief.
If the state you read suggests an action - a verified pull request or merge request ready to merge, a queued item whose gate has arrived, or a needs-decision finding - name it in its section and leave the action to the normal lifecycle and configured authority rather than taking it from inside this skill.
